...
import numpy as np
from drag_utils_real import DragStuff
import torch as th
import threading
...
class App:
    def __init__(self):
        ...
        self._progres_bar.value = 100
        self._progres_label = gui.Label("Progress 100%  ")
    ...
    def _create_mesh_callback(self):
        if self._model_path.selected_index:
            ...
            def create_mesh():
                ...
                self.drag_stuff.mesh = o3d.io.read_triangle_mesh('datas/real-shape-edit/planes/test10/edit01.obj')
                self.drag_stuff.mesh0 = copy.deepcopy(self.drag_stuff.mesh)
                self.update_mesh(self.drag_stuff.mesh, color=self.current_color)
    ...
    def _reset_mesh_callback(self):
        if self.drag_stuff.mesh0 is None:
            ...
            # self.target_pnt.clear()
            # self.draw_source_flag = True
            self.update_mesh(mesh, color=self.current_color)
        else:
            name_list = (['start%d' % i for i in range(1, len(self.source_pnt) + 1)] +
                         ['end%d' % i for i in range(1, len(self.target_pnt) + 1)] +
                         ['line%d' % i for i in range(1, len(self.target_pnt) + 1)])
            ...
            self.update_mesh(self.drag_stuff.mesh0, color=self.current_color)
    ...
    def _inversion_callback(self):
        self._print_label_text = 'DDPM Inversion...'
        gui.Application.instance.post_to_main_thread(self.window, self._print_label_text_fun)
    ...
    def _train_start_callback(self):
        ...
        def training():
            ...
            for progress_value in self.drag_stuff.training(
                ...
                gui.Application.instance.post_to_main_thread(self.window, self._set_progress_value_fun)
            # self.drag_stuff.mesh = o3d.io.read_triangle_mesh('datas/generated-shape-edit/cars/edit51.obj')
            self.update_mesh(self.drag_stuff.mesh, update_camera=False, color=self.current_color)
    ...
    def update_mesh(self, mesh, update_camera=True, color=None):
