...
import numpy as np
from drag_utils_comparison import DragStuff
import torch as th
import threading
...
class App:
        ...
        if self.drag_geometry:
            ...
            for name, mesh in self.drag_geometry.items():
                if name.startswith('keypoint'):
                    if int(name[8:]) in self.select_key:
                        continue
                v = np.asarray(mesh.vertices)
                f = np.asarray(mesh.triangles)
                v_total.append(v)
                f_total.append(f+start)
                if name.startswith('start') or name.startswith('keypoint'):
                    c_total.append(np.tile(np.array([1, 0, 0], dtype=np.uint8), (v.shape[0], 1)))
                elif name.startswith('end'):
                    c_total.append(np.tile(np.array([0, 0, 1], dtype=np.uint8), (v.shape[0], 1)))
                else:
                    c_total.append(np.tile(np.array([0, 1, 0], dtype=np.uint8), (v.shape[0], 1)))
                start += v.shape[0]
            ...
            with open(os.path.join(os.path.dirname(filename), 'EditLog'), 'a+') as f:
                f.write('Edit'+file+':'+'\n')
                f.write('keypoint number:' + ', '.join([str(i) for i in self.select_key]) + '     delta:' +
                        ', '.join([str(i) for i in self._vedit.vector_value]) + '\n')
                # f.write(' '.join(['keypoint number:'] + [str(i) for i in self.select_key] + ['     delta:'] +
                #                  [str(i) for i in self._vedit.vector_value]) + '\n')
    ...
    def _load_mesh_done(self, filename):
        if filename.endswith((".obj", ".ply", ".off", ".stl")):
            ...
            self.drag_stuff.clear_params()
            self.update_mesh(mesh)
            self.rela_path = os.path.dirname(filename)
            encode = np.load(os.path.join(os.path.dirname(filename), 'reconstructed_pcd.npz'))
            keypoint = encode['keypoint'][0]
            self.keypoint = keypoint
            for i, point in enumerate(keypoint):
                self.draw_point(point, rgb=(0.5, 0.5, 0.5), name='keypoint'+str(i))
            self.window.close_dialog()
            os.chdir(self.abs_dir)
        else:
    ...
    def _inversion_callback(self):
        ...
        def mesh_inversion():
            t = threading.Thread(target=self.drag_stuff.train_triplane, args=(os.path.join(self.rela_path, 'tri_feat.npy'), ))
            t.start()
            t.join()
    ...
    def _mouse_event(self, event):
        if (event.type == gui.MouseEvent.Type.BUTTON_DOWN and event.is_button_down(gui.MouseButton.LEFT) and
            ...
            def depth_callback(depth_image):
                ...
                if self.draw_source_flag:
                    distance = np.linalg.norm(picked_point - self.keypoint, axis=1)
                    if np.min(distance) < 0.08:
                        picked_point = self.keypoint[np.argmin(distance)]
                        print("###keypoint", np.argmin(distance), end=" ")
                        self.select_key.append(np.argmin(distance))
