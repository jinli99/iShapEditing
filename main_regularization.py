...
import numpy as np
from drag_utils_regularization import DragStuff
import torch as th
import threading
...
class App:
    def __init__(self):
        ...
        self._tw = gui.TextEdit()
        # self._drag_panel.add_child(self._tw)
        ...
        self._tw.set_on_value_changed(self._tw_change_callback)
    ...
    def _tw_change_callback(self, new_text):
        self.drag_stuff.args.w_time = int(new_text)
        print('using tw:', new_text)
    ...
    def _reset_mesh_callback(self):
        if self.drag_stuff.mesh0 is None:
            ...
            #  don't used when test on the effect of regularization!!!
            # name_list = (['start%d' % i for i in range(1, len(self.source_pnt) + 1)] +
            #              ['end%d' % i for i in range(1, len(self.target_pnt) + 1)] +
            #              ['line%d' % i for i in range(1, len(self.target_pnt) + 1)])
            # self.remove_geometry_name(name_list)
    ...
    def _inversion_callback(self):
        ...
        def mesh_inversion():
            t = threading.Thread(target=self.drag_stuff.train_triplane, kwargs={"tri_feat_path":os.path.join(self.rela_path, 'tri_feat.npy')})
            t.start()
            t.join()
